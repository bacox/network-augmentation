...
class VGG(nn.Module):
    ...
    def __init__(
        ...
        classifier_size: int = 2
    ) -> None:
        super(VGG, self).__init__()
        self.features = features
        self.avgpool = nn.AdaptiveAvgPool2d((7, 7))
        self.classifier = classifier_builder(classifier_size, num_classes)
        # else:
...
def classifier_builder(size: int, num_classes: int):
    ...
    layers = stem + block_fcl * size + tail
    return nn.Sequential(*layers)
...
def construct_vgg_variant(conv_variant: int, fcl_variant: int, batch_norm: bool, pretrained: bool, progress: bool, **kwargs: Any) -> VGG:
    ...
    model = VGG(make_layers(features, batch_norm=batch_norm), classifier_size=fcl_variant, **kwargs)
    if pretrained:
